...
if ( !function_exists('pagelist_unqprfx_ext_shortcode') ) {
	function pagelist_unqprfx_ext_shortcode( $atts ) {
		...
		if( count( $list_pages ) == 0 ){ // if there is no subpages
			// No pages is showing, midified by Jonas Nyhav 20140505
		...
		foreach($list_pages as $page){
			...
			if ( ( !empty( $offset ) && is_numeric( $offset ) && $offset_count > $offset ) || ( empty( $offset ) ) || ( !empty( $offset ) && !is_numeric( $offset ) ) ) {
				if ( ( !empty( $number ) && is_numeric( $number ) && $count <= $number ) || ( empty( $number ) ) || ( !empty( $number ) && !is_numeric( $number ) ) ) {
					...
					$css_background_color = get_post_custom_values('background-color', $page->ID);
					if ($css_background_color[0]!="") {$css_background_color = " background-color: ".$css_background_color[0]."; ";}
					
					
					$css_background_image = wp_get_attachment_image_src(get_post_thumbnail_id($page->ID), 'background' );
					if ($css_background_image[0]!="") {$css_background_image = " background-image: url(".$css_background_image[0]."); ";}
					
					
					$css_color = get_post_custom_values('color', $page->ID);
					if ($css_color[0]!="") {$css_color = " color: ".$css_color[0]."; ";}

					
					$list_pages_html .= '<div class="site-inner onepage-section site-inner-part page-list-part page-list-item page-list-ext-item" id="site-inner-part-'.$count.' onepage-'.$page->ID.'" style="'.$css_color.$css_background_color.$css_background_image.'">';
					...
					if( $show_image == 1 ){
						if ( function_exists( 'get_the_post_thumbnail' ) ) { // if we have WordPress 2.9+
							if( get_the_post_thumbnail( $page->ID ) ){ // if there is a featured image
								...
								$list_pages_html .= '<a href="'.$link.'" title="'.esc_attr($page->post_title).'">';
								$list_pages_html .= '<div class="page-list-ext-image outer-image" style="background-image: url('.$img_url.');">';
								$list_pages_html .= '<img class="feature-image" src="'.$img_url.'" width="'.$image_width.'" alt="'.esc_attr($page->post_title).'" />'; // not using height="'.$image_height.'" because images could be not square shaped and they will be stretched
								$list_pages_html .= '<div class="overlay" style="background-image: url('.get_stylesheet_directory_uri().'/images/overlay.png);" ></div>'; // not using height="'.$image_height.'" because images could be not square shaped and they will be stretched
								$list_pages_html .= '</div></a>';
							}else{
								if( $show_first_image == 1 ){
									$img_scr = pagelist_unqprfx_get_first_image( $page->post_content );
									if( !empty( $img_scr ) ){
										$list_pages_html .= '<div class="page-list-ext-image outer-image"><a href="'.$link.'" title="'.esc_attr($page->post_title).'">';
					...
					$list_pages_html .= '<div class="wrap"><div class="onepage-wrap">';
					$list_pages_html .= '<main class="content" role="main" itemprop="mainContentOfPage">';
					$list_pages_html .= '<article class="entry page type-page type-onepage status-publish post-'.$page->ID.'" itemscope="itemscope" itemtype="http://schema.org/CreativeWork">';
					
					if( $show_image == 1 ){
						if ( function_exists( 'get_the_post_thumbnail' ) ) { // if we have WordPress 2.9+
							if( get_the_post_thumbnail( $page->ID ) ){ // if there is a featured image
								$list_pages_html .= '<div class="page-list-ext-image inner-image"><a href="'.$link.'" title="'.esc_attr($page->post_title).'">';
					...
					if( $show_title == 1 ){
						$list_pages_html .= '<header class="entry-header"><h2 class="entry-title page-list-ext-title"><a href="'.$link.'" title="'.esc_attr($page->post_title).'" style="'.$css_color.'">'.$page->post_title.'</a></h2></header>';
						//$list_pages_html .= '<h3 class="page-list-ext-title">'.$page->post_title.'</h3>';
					...
					if( $show_content == 1 ){
						...
						if( !empty( $page->post_excerpt ) ){
							...
							$text_content = do_shortcode($page->post_content);
						}
						...
						if ( post_password_required($page) ) {
							...
							if( $show_title == 0 ){ // make content as a link if there is no title
								$content = '<a href="'.$link.'" style="'.$css_color.'">'.$content.'</a>';
							}
						}

						$list_pages_html .= '<div class="entry-content page-list-ext-item-content" itemprop="text">'.$content.'</div>';
					...
					if( $show_meta_key != '' ){
						...
						if( !empty($post_meta) ){ // hide empty
							...
							if($meta_pos === false) { // %meta% not found in template
								...
								$list_pages_html .= '<div class="page-list-ext-meta page-list-ext-meta-'.$show_meta_key.'">'.$meta_template_html.'</div>';
					...
					$list_pages_html .= '</article></main></div></div></div>'."\n";
				}
			}
		}
		$return = $pagelist_unqprfx_powered_line;
